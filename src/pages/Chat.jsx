...
const Chat = () => {
  ...
  return (
    ...
    <div
      className="overflow-y-hidden w-full h-screen p-3 bg-stone-900 flex flex-col items-center justify-center"
      style={{ backgroundImage: "url(/bg.jpg)" }}
    >
      {isLoading && (
        <div className="fixed top-0 left-0 w-full h-screen bg-stone-950/90 flex items-center justify-center z-40">
          <div className="border-4 border-stone-200 border-t-rose-500 size-12 animate-spin rounded-full"></div>
        </div>
      )}
      {/*
      ...
      */}
      <div className="w-full flex-1 min-h-0 rounded-xl bg-stone-800/70 backdrop-blur-md text-white shadow-xl flex flex-col items-start justify-start overflow-hidden">
        {/* Header */}
        <div className="w-full h-20 border-b border-stone-400 shadow-md flex items-center justify-between px-4 shrink-0">
          ...
          <h1 className="text-xl font-semibold">ComfyAI (v2.1)</h1>
          <button
            ...
            >
              <div
                className={`max-w-xs md:max-w-md lg:max-w-lg px-4 py-2 rounded-xl shadow font-semibold ${
                  msg.sender === "user"
                    ? "bg-rose-600 text-white border border-black"
          ...
          <div className="w-full h-auto min-h-[3.5rem] border-t border-stone-400 shadow-md p-2 shrink-0">
            ...
            >
              <input
                ...
                className="flex-1 bg-stone-700 border border-stone-400 p-2 rounded-xl outline-none disabled:cursor-not-allowed focus-within:ring-1 ring-rose-500 disabled:opacity-50 placeholder:text-rose-300"
              />
              <button
                type="submit"
                disabled={isLoading || !userInput.trim() || !conversationId}
                className="px-8 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? (
                  <svg
